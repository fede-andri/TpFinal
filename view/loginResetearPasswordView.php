<!doctype html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <link rel="stylesheet" href="view/css/headerFooterStyle.css">

    <title>Transporte San Justo</title>
</head>
<header >
    <div id="header">
        <div >
            <img src="images/logo-transporte2.png" height="150px" width="150px" alt="Logo" >
        </div>
    </div>
</header>
<body>
<div class="w3-panel w3-blue-gray w3-display-middle w3-center">
    <h3>Recuperacion de contraseña !</h3><br>
        <label>ingrese su correo electronico</label><br>
        <input type="text" id="email" name="email">


    <p>Un Email de recuperacion sera enviado, desea continuar?</p>
    <br>
    <a class="w3-button w3-orange" href="index.php?module=login&action=index">Cancelar</a>
    <a class="w3-button w3-orange" href="index.php?module=login&action=resetearPassword&email=email">Aceptar</a>
</div>

{{>footer}}
